...
public enum DeviceInfo {
    ...
    private static var allPreferencesPaths: [String] {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        let prefDir = "\(home)/Library/Containers/com.kakao.KakaoTalkMac/Data/Library/Preferences"
        let container = ((try? FileManager.default.contentsOfDirectory(atPath: prefDir)) ?? [])
            .filter { $0.hasPrefix("com.kakao.KakaoTalkMac") && $0.hasSuffix(".plist") }
            .map { "\(prefDir)/\($0)" }
        return Array(Set(container + [preferencesPath])).sorted()
    }
    ...
    public static func userId() throws -> Int {
        let plistPaths = allPreferencesPaths
    ...
    public static func candidateUserIds() -> [Int] {
        ...
        for plistPath in plistPaths {
            ...
            result += ids.compactMap { item -> Int? in
    ...
    public static func discoverDatabaseFiles() -> [String] {
        ...
        let pattern = try! NSRegularExpression(pattern: "^[0-9a-f]{78}(\\.db)?$")
        return entries.filter { entry in
            let range = NSRange(entry.startIndex..., in: entry)
            return pattern.firstMatch(in: entry, range: range) != nil
        }.sorted().map { "\(containerPath)/\($0)" }
    }
