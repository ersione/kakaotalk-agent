...
struct AXWatchCommand: ParsableCommand {
    ...
    @Flag(name: .long, help: "Output each event as one compact JSON line (NDJSON)")
    var json: Bool = false
    ...
    func run() throws {
        ...
        // Watch only needs a short overlapping tail. Parsing the whole exposed
        // transcript makes large rooms take tens of seconds per scan.
        ...
        state.replaceBaseline(with: baseline.messages)

        if json {
            try emitReady(chat: currentChatTitle, detectedAt: baseline.fetchedAt)
        }
        ...
        while !watchWasInterrupted() {
            ...
            let emitted = state.consume(snapshotMessages: snapshot.messages)
    ...
    private func emitReady(chat: String, detectedAt: Date) throws {
        let formatter = ISO8601DateFormatter()
        formatter.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
        let payload = WatchJSONEvent(
            chat: chat,
...
struct WatchPollingState {
    ...
    mutating func consume(snapshotMessages: [TranscriptMessage]) -> [TranscriptMessage] {
        ...
        // Losing the overlap means the AX context/window was rebuilt. Treat the
        // whole snapshot as a new baseline; emitting it would replay history.
        guard overlap.endIndex >= 0 else {
            previousMessages = currentMessages
            return []
        }
        let emitted = Array(currentMessages.dropFirst(overlap.endIndex + 1))
