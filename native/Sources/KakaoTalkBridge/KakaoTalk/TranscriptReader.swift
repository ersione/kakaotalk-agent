...
struct KakaoTalkTranscriptReader {
    ...
    private func extractMessages(
        ...
        for (offset, analysis) in analyses.enumerated() {
            ...
            // A left-aligned bubble is incoming even when KakaoTalk omits the
            // repeated sender label. Preserve that distinction; nil encodes as
            // "(me)" and would otherwise make the agent discard every message.
            let author = resolvedAuthor.author ?? (side == .left ? "(unknown)" : nil)
