...
struct ChatWindowResolver {
    ...
    func resolve(query: String) throws -> ChatWindowResolution {
        ...
        // KakaoTalk masks detached window titles and their children while the
        // app is backgrounded on recent builds. Activate once before falling
        // back to the much more invasive main-window search flow.
        kakao.activate()
        Thread.sleep(forTimeInterval: 0.25)
        if let existingWindow = findMatchingChatWindow(in: kakao.windows, query: query) {
            standardizeReadableWindow(existingWindow, label: "activated existing chat window")
            return ChatWindowResolution(window: existingWindow, method: .existingWindow)
        }
