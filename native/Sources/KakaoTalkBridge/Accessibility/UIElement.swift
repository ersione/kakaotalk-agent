...
public final class UIElement: @unchecked Sendable {
    ...
    public var parent: UIElement? {
        copyElementAttribute(kAXParentAttribute)
    }

    public var children: [UIElement] {
        copyElementArrayAttribute(kAXChildrenAttribute)
    }

    public var windows: [UIElement] {
        copyElementArrayAttribute(kAXWindowsAttribute)
    }

    public var focusedWindow: UIElement? {
        copyElementAttribute(kAXFocusedWindowAttribute)
    }

    public var focusedUIElement: UIElement? {
        copyElementAttribute(kAXFocusedUIElementAttribute)
    }

    public var mainWindow: UIElement? {
        copyElementAttribute(kAXMainWindowAttribute)
    }

    // AXUIElement is a CFType. Swift 6.2 can incorrectly bridge it through the
    // generic attribute<T> path (returning the application root for every child).
    // Keep element-valued attributes on a concrete CoreFoundation conversion path.
    private func copyElementAttribute(_ name: String) -> UIElement? {
        var value: CFTypeRef?
        guard AXUIElementCopyAttributeValue(axElement, name as CFString, &value) == .success,
              let value,
    ...
    private func copyElementArrayAttribute(_ name: String) -> [UIElement] {
        var value: CFTypeRef?
        guard AXUIElementCopyAttributeValue(axElement, name as CFString, &value) == .success,
              let value,
              CFGetTypeID(value) == CFArrayGetTypeID()
        else { return [] }
        // The concrete cast is reliable; the generic attribute<T> cast is not.
        let elements = value as! [AXUIElement]
        return elements.map(UIElement.init)
    }
